  ...
  public partial class Action
  {
    public static void CombatStart(Character participant, Character target)
    {
      Debug.Log("CombatStart triggered");
        ...
        //   // console.log("environment items = ${location.items[0].name} and ${location.items[1].name} and ${location.items[2].name} and ${location.items[3].name} and ${location.items[4].name}. Inside in ${location.items[4].name}: ${location.items[4].contents[0].name}");
